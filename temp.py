...
def draw_repeat_points(cell_size):
    ...
    for el in repeat_points: 
        ...
        plt.plot(x, y, 'red', linewidth=0.1)

@nb.njit(cache=True)
def calc_cell_size(X, Y,  matrix_size):
    """ Рассчет размера клетки: массив точек по Х, массив точек по У, размер матрицы """
    cell_size = np.array([((np.max(X) - np.min(X)) / (matrix_size - 1)), ((np.max(Y) - np.min(Y)) / (matrix_size - 1))])
    return np.max(cell_size)

def error_len(x, y, grid):
    return not (x > (len(grid) - 1) or y > (len(grid) - 1) or x < 0 or y < 0)

def save_coord_point(coord_point):
    """ Сохранение точек матрицы в массив: массив координат """
    coord_point_array.append(coord_point)
    return coord_point_array

def update_grid_and_repeat_points(grid, point_coords, k, new_value):
    """
    Обновляет grid и repeat_points с новым значением для точки с координатами point_coords и индексом k.
    """
    y, x = point_coords
    if grid[y][x][0][k] in {0, new_value}:
        grid[y][x][0][k] = new_value
        grid[y][x][1] += 1
    else:
        point_temp = grid[y][x][0].copy()  # Используем NumPy для копирования
        point_temp[k] = new_value
        if not any(np.array_equal(point_temp, rp[3]) for rp in repeat_points):
            repeat_points.append([y, x, grid[y][x][0].copy(), point_temp])
...
def comparison_point(arr, grid):
    ...
    for i in nb.prange(1, len(arr) - 1):
        ...
        if arr[i-1][0] < arr[i][0]:
            ...
            k = 1
        if arr[i+1][1] < arr[i][1]:
            update_grid_and_repeat_points(grid, arr[i], k, 1)
        elif arr[i+1][1] > arr[i][1]:
            update_grid_and_repeat_points(grid, arr[i], k, 3)
        if arr[i+1][0] < arr[i][0]:
            update_grid_and_repeat_points(grid, arr[i], k, 4)
        elif arr[i+1][0] > arr[i][0]:
            update_grid_and_repeat_points(grid, arr[i], k, 2)
...
def create_grid(matrix_size, func_1, func_2, cell_size=0):
    ...
    segment_start_point = np.array([[-2.6, 2.6], [-4.3, 4.3]])
    start_point = calc_start_point(quantity_start_point, segment_start_point)
    mu = 0.1
    ...
    try:
        ...
        for j in nb.prange(len(start_point)):
            ...
            if(cell_size == 0):
                nb_point = np.array(point)
                cell_size = calc_cell_size(nb_point[0], nb_point[1], matrix_size)
